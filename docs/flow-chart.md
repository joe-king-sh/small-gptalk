...
flowchart TD
  ...
  L2 --> L3{"レッスンルームの有無"}
  L3 -->|レッスンがない| S1(["SmallGPTalkの説明をする、講師を呼ぶからちょっと待ってね"]):::systemMessage
  ...
  subgraph レッスン終了
    ...
    G3 --> S2(["SmallGPTalkを遊んでくれてありがとう、楽しかったかな？またやりたかったら「はじめる」と話しかけてね。"]):::systemMessage
    S2 --> END
  end

  L3 -->|レッスンルームがある| L8{レッスンの長さ判定}
  L8 -->|30分を超えている| G3(["ここを直すともっと良くなるよ。またやろうね。"]):::gptMessage
  L8 -->|30分を超えていない| G2
